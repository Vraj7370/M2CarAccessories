    ...
    <AnimatePresence>
      {!done && (
        ...
        >
          ...
          {/* ═══════════════════════════════════════ */}
          {/* 3D LOGO BOX — img replaces M2 text     */}
          {/* ═══════════════════════════════════════ */}
          <motion.div
          ...
          >
            ...
            >
              ...
              {/* ── ACTUAL LOGO IMAGE (same as Navbar) ── */}
              <img
                src="/m2-logo.png"
                ...
                style={{
                  ...
                  mixBlendMode: 'screen',
                  filter: logoHovered
                    ? 'brightness(1.15) contrast(1.1) drop-shadow(0 0 14px rgba(201,168,76,0.5))'
                    : 'brightness(1.08) contrast(1.1) drop-shadow(0 0 10px rgba(201,168,76,0.25))',
              ...
              {/* Corner bracket accents inside box */}
              {[
                ...
                <span
                  key={i}
                  className={`absolute w-3 h-3 border-[#C9A84C]/40 z-4 ${cls}`}
                />
              ))}
            </div>

            {/* "Car Accessories" tagline */}
            <motion.p
          ...
          {particles.current.map((pt) => (
            <motion.div
              key={pt.id}
              className="absolute rounded-full"
              style={{ left: pt.left, bottom: pt.bottom, width: pt.size, height: pt.size, background: '#C9A84C' }}
              animate={{ y: [0, -600], opacity: [0, 0.7, 0.3, 0] }}
              transition={{ duration: pt.duration, repeat: Infinity, delay: pt.delay }}
            />
          ))}
        </motion.div>
      )}
